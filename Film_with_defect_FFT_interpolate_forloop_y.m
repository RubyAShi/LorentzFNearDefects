...
save('Ym20to20um.mat','hzk', 'kx','ky', 'x', 'y', 'a', 'b', 'alpha', 'x0', 'y0', 'xPrime');
...
% convert to force
I = 3e-3; % A
YForce = 2 * Phi_0 * real_hz_useful_interp * 1e6 * I * 1e15; %fN

figure(i)
imagesc(xq, yq, YForce)
colormap jet
%colorbar 
c = colorbar;
c.Label.String = 'fN';
...
y = 1i .* ky .* exp(-Q * z_0 - 1i * qx * x0 - 1i * ky * y0) .* besselj(1, Q * a) .* (qx .* kx + ky.^2) .* cos((kx - qx)*xPrime) ./ (k .* (1 + lambda_0 * Q) .* (1 + lambda_0 * k).* Q);
end
